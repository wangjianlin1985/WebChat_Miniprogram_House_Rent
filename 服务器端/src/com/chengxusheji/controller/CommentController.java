...
public class CommentController extends BaseController {
	...
	public String szFrontlist(@ModelAttribute("minsuObj") Minsu minsuObj,@ModelAttribute("userObj") UserInfo userObj,String commentTime,Integer currentPage, Model model, HttpServletRequest request, HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (commentTime == null) commentTime = "";
		String ownerUserName = (String) session.getAttribute("user_name");
		List<Comment> commentList = commentService.szQueryComment(ownerUserName, minsuObj, userObj, commentTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    commentService.szQueryTotalPageAndRecordNumber(ownerUserName, minsuObj, userObj, commentTime);
	    /*获取到总的页码数目*/
	    int totalPage = commentService.getTotalPage();
	    /*当前查询条件下总记录数*/
	    int recordNumber = commentService.getRecordNumber();
	...
	public void OutToExcel(@ModelAttribute("minsuObj") Minsu minsuObj,@ModelAttribute("userObj") UserInfo userObj,String commentTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        String[] headers = { "评论id","被评民宿","评论内容","评论用户","评论时间"};
